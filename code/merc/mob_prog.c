 ...
 * Called by cmd_eval()
 */
static bool num_eval(long lval, long oper, long rval)
{
    switch (oper) {
	case EVAL_EQ:
	    return lval == rval;
...
static int count_people_room(struct char_data *mob, int iFlag)
{
	...
	if (mob != vch
		&& (iFlag == 0
			...
			&& mob->mob_idx->vnum == vch->mob_idx->vnum)
...
static int get_order(struct char_data *ch)
{
	...
	if (vch == ch)
	    return i;
	if (IS_NPC(vch)
		&& vch->mob_idx->vnum == ch->mob_idx->vnum)
	    i++;
    }
    return 0;
}
...
static bool has_item(struct char_data *ch, long vnum, int item_type, bool fWear)
{
	...
	if ((vnum < 0 || obj->objtemplate->vnum == vnum)
		&& (item_type < 0 || obj->objtemplate->item_type == item_type)
...
static bool cmd_eval(long vnum, const char *line, int check, struct char_data *mob, struct char_data *ch, const void *arg1, const void *arg2, struct char_data *rch)
{
	...
	case CHK_HAS:
	    return lval_char != NULL && has_item(lval_char, -1, (int)item_lookup(buf), false);
	case CHK_USES:
	    return lval_char != NULL && has_item(lval_char, -1, (int)item_lookup(buf), true);
	case CHK_NAME:
	    switch (code) {
		default:
		case 'i':
	...
	case CHK_VNUM:
		...
		case 'q':
			...
			lval = lval_char->mob_idx->vnum;
		    break;
		case 'o':
		case 'p':
		    if (lval_obj != NULL)
			lval = lval_obj->objtemplate->vnum;
	    }
	    break;
	case CHK_HPCNT:
	...
	case CHK_MONEY:  /* Money is converted to silver... */
	    if (lval_char != NULL)
		lval = (long)(lval_char->gold + (lval_char->silver * 100)); break;
	case CHK_OBJVAL0:
...
void mp_give_trigger(struct char_data *mob, struct char_data *ch, struct gameobject *obj)
{
	...
	if (prg->trig_type == TRIG_GIVE) {
	     ...
	     */
	    if (is_number(p)) {
		if (obj->objtemplate->vnum == parse_long(p)) {
