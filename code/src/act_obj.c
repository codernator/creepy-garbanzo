...
*	do_get
***************************************************************************/
void do_get2(CHAR_DATA *ch, char *argument)
{
...
void do_get(CHAR_DATA *ch, char *argument)
{
	...
	if (arg2[0] == '\0') {
		...
		case ITEM_CORPSE_PC:
		{
			...
			/* this code will need to be changed for new
			 * configuration. */
			if ((str_cmp(container->owner, ch->name))
			    && !IS_IMMORTAL(ch)
			    && (container->value[0] > 2)
			    && container->owner != NULL) {
				send_to_char("Nothing more can be looted from that.\n\r", ch);
				return;
			}
...
void do_exchange(CHAR_DATA *ch, char *argument)
{
	...
	for (mob = ch->in_room->people; mob; mob = mob->next_in_room) {
		if (IS_NPC(mob)
		    && IS_SET(mob->act, ACT_IS_EXCHANGER))
			break;
	}

	if (mob == NULL) {
		send_to_char("You must find the commissary to exchange your gold.\n\r", ch);
		return;
	}
	...
	if (arg[0] == '\0') {
		send_to_char("Exchange?  Try `^exchange list`` or `^exchange buy``.\n\r", ch);
		return;
	}

	if (!str_prefix(arg, "list")) {
		send_to_char("Current Items available for `^Exchange``:\n\r", ch);
		send_to_char("----------------------------------------------------\n\r", ch);
		send_to_char("One `8Skillset`` Token     20,000,000 `#gold`` pieces\n\r", ch);
		...
		send_to_char("One `!Train``               3,000,000 `#gold`` pieces\n\r", ch);
		send_to_char("One `^Wild`` Token          1,000,000 `#gold`` pieces\n\r", ch);
		send_to_char("To buy an item, type 'EXCHANGE BUY <item>'.\n\r", ch);
		return;
	}
	...
	if (!str_prefix(arg, "buy")) {
		...
		if (!str_prefix(item, "skillset")) {
			if (ch->pcdata->gold_in_bank < 20000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}

			send_to_char("You exchange rp,000,000 `#gold`` pieces for one `8Skillset`` token!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 20000000;
			ch->pcdata->skillset += 1;
			return;
		}

		if (!str_prefix(item, "damagenoun") || !str_prefix(item, "damnoun")) {
			if (ch->pcdata->gold_in_bank < 15000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}

			send_to_char("You exchange 15,000,000 `#gold`` pieces for one `PDamagenoun`` token!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 15000000;
			ch->pcdata->damnoun += 1;
			return;
		}

		if (!str_prefix(item, "fireproof")) {
			if (ch->pcdata->gold_in_bank < 10000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}
			send_to_char("You exchange 10,000,000 `#gold`` pieces for one `!Fireproof`` token!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 10000000;
			ch->pcdata->fireproof += 1;
			return;
		}

		if (!str_prefix(item, "200/200") || !str_prefix(item, "twohundred")) {
		...
		if (!str_prefix(item, "weaponflag")) {
			if (ch->pcdata->gold_in_bank < 10000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}
			send_to_char("You exchange 10,000,000 `#gold`` pieces for one `OWeaponflag`` token!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 10000000;
			ch->pcdata->weaponflag += 1;
			return;
		}

		if (!str_prefix(item, "ac")) {
			if (ch->pcdata->gold_in_bank < 5000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}
			send_to_char("You exchange 5,000,000 `#gold`` pieces for one `^AC`` token!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 5000000;
			ch->pcdata->armorclass += 1;
			return;
		}

		if (!str_prefix(item, "imp")) {
			if (ch->pcdata->gold_in_bank < 3000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}

			send_to_char("You exchange 3,000,000 `#gold`` pieces for one `5Imp`` token!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 3000000;
			ch->pcdata->imp += 1;
			return;
		}

		if (!str_prefix(item, "wild")) {
			if (ch->pcdata->gold_in_bank < 1000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}

			send_to_char("You exchange 1,000,000 `#gold`` pieces for one `^Wild`` token!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 1000000;
			ch->pcdata->immwild += 1;
			return;
		}

		if (!str_prefix(item, "train")) {
			if (ch->pcdata->gold_in_bank < 3000000) {
				send_to_char("You don't have enough gold!\n\rRemember, we only take gold from your bank account.", ch);
				return;
			}

			send_to_char("You exchange 3,000,000 `#gold`` pieces for one `!Train``!.\n\r", ch);
			ch->pcdata->gold_in_bank -= 3000000;
			ch->pcdata->train += 1;
			return;
		}

		send_to_char("We do not exchange for that.  Try 'exchange list'.\n\r", ch);
		return;
	}

	send_to_char("Try 'exchange list' or 'exchange buy'.\n\r", ch);
	return;
}
...
*	do_put
***************************************************************************/
void do_put2(CHAR_DATA *ch, char *argument)
{
	char arg1[MAX_INPUT_LENGTH];
	char arg2[MAX_INPUT_LENGTH];
	char rest[MAX_INPUT_LENGTH];
	...
	if (str_cmp(arg1, "all") && str_prefix("all.", arg1)
		...
		if ((long)(get_obj_weight(obj) + get_true_weight(container)) > (container->value[0] * 10)
		    || (long)(get_obj_weight(obj)) > (container->value[3] * 10)) {
...
*	do_drop
***************************************************************************/
void do_drop2(CHAR_DATA *ch, char *argument)
{
	...
	if (is_number(arg)) {
		/* 'drop NNNN coins' */
		unsigned int amount, gold = 0, silver = 0;
...
*	do_give
***************************************************************************/
void do_give2(CHAR_DATA *ch, char *argument)
{
	char arg1 [MAX_INPUT_LENGTH];
	char arg2 [MAX_INPUT_LENGTH];
	char rest[MAX_INPUT_LENGTH];
...
*	do_vorpalize
***************************************************************************/
void do_vorpalize(CHAR_DATA *ch, char *argument)
{
	OBJ_DATA *obj;
	AFFECT_DATA af;
	SKILL *skill_vorpalize;
	int percent;
	int skill;

	if ((skill_vorpalize = skill_lookup("vorpalize")) == NULL) {
		send_to_char("Huh?", ch);
		return;
	}
	...
	if ((skill = get_learned_percent(ch, skill_vorpalize)) < 1) {
		send_to_char("Are you crazy? You'd kill yourself!\n\r", ch);
		return;
	}
	...
	if (obj->item_type == ITEM_WEAPON) {
		...
		if (percent < skill) {
			af.where = TO_WEAPON;
			af.type = skill_vorpalize->sn;
			af.skill = skill_vorpalize;
			af.level = (int)(ch->level * percent / 100);
			af.duration = (int)(ch->level / 2 * percent / 100);
			af.location = 0;
			af.modifier = 0;
			...
			act("$n concentrates and vorpalizes $p.", ch, obj, NULL, TO_ROOM);
			act("$p now wields with deadly force.", ch, obj, NULL, TO_CHAR);
			check_improve(ch, skill_vorpalize, TRUE, 3);
			WAIT_STATE(ch, skill_vorpalize->wait);
			return;
		} else {
			act("You fail to vorpalize $p.", ch, obj, NULL, TO_CHAR);
			check_improve(ch, skill_vorpalize, FALSE, 3);
			WAIT_STATE(ch, skill_vorpalize->wait);
			return;
		}
	}

	act("You can't vorpalize $p.", ch, obj, NULL, TO_CHAR);
	return;
}
...
*   do_objident
***************************************************************************/
void do_objident(CHAR_DATA *ch, char *argument)
{
	if (IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP)) {
