...
void do_dirt(CHAR_DATA *ch, char *argument)
{
	...
	if ((skill = skill_lookup("dirt kicking")) == NULL) {
		send_to_char("Huh?\n\r", ch);
		return;
	}
...
void do_trip(CHAR_DATA *ch, char *argument)
{
	...
	if ((skill = skill_lookup("trip")) == NULL) {
		send_to_char("Huh?\n\r", ch);
		return;
	}
	...
	if ((chance = get_learned_percent(ch, skill)) <= 0
		...
		send_to_char("```#T```Pr```@i```^p```#p```Pi```@n```^g``?  Aren't we all?\n\r", ch);
		return;
	}
...
void do_assassinate(CHAR_DATA *ch, char *argument)
{
	...
	if (ch->fighting != NULL) {
		send_to_char("You can't kill someone whois fighting.\n\r", ch);
		return;
	} else if ((victim = get_char_room(ch, arg)) == NULL) {
		send_to_char("They aren't here.\n\r", ch);
		return;
	...
	if ((obj = get_eq_char(ch, WEAR_WIELD)) == NULL) {
		send_to_char("You need a weapon to assassinate.\n\r", ch);
		return;
	}

	if (!IS_NPC(victim)) {
		sprintf(log_buf, "%s is attempting to assassinate %s",
			ch->name, victim->name);
		log_string(log_buf);
	}

	check_killer(ch, victim);
	WAIT_STATE(ch, gsp_assassinate->wait);

	if (number_percent() < get_learned_percent(ch, gsp_assassinate)
	    || !IS_AWAKE(victim)) {
		check_improve(ch, gsp_assassinate, TRUE, 1);
		multi_hit(ch, victim, gsp_assassinate->sn);
		vorpal_effect(ch, victim, obj);
	} else {
		check_improve(ch, gsp_assassinate, FALSE, 1);
		damage(ch, victim, 0, gsp_assassinate->sn, DAM_NONE, TRUE);
	}
...
void do_bludgeon(CHAR_DATA *ch, char *argument)
{
	...
	if ((skill = gsp_bludgeon) == NULL) {
		send_to_char("Huh?\n\r", ch);
		return;
	}

	if (!IS_NPC(ch) && (percent = get_learned_percent(ch, skill)) <= 0) {
		send_to_char("`!WHAT\n\r`PTHE\n\r`OFUCK?``\n\r", ch);
		return;
	}
	...
	if (ch->fighting != NULL) {
		send_to_char("You're already fighting, retard.\n\r", ch);
		return;
	}
	...
	if (!IS_AWAKE(victim)) {
		send_to_char("Give me a break, they are already sleeping.", ch);
		return;
	}
	...
	if (ch->fighting == victim) {
		send_to_char("You try to sneak up behind them, but they're fighting you, retard.\n\r", ch);
		return;
	}

	if (!IS_NPC(victim)) {
		if ((ch->level + 10) < victim->level) {
			act("You failed to knock $N out!\n\r", ch, NULL, victim, TO_CHAR);
			act("$n tries to knock you out, but fails!", ch, NULL, victim, TO_VICT);
			damage(ch, victim, 0, skill->sn, DAM_NONE, TRUE);
			return;
		}
	}
	...
	if (percent < number_percent()) {
		send_to_char("You failed to knock your victim out!\n\r", ch);
		damage(ch, victim, 0, skill->sn, DAM_NONE, TRUE);
		return;
	}


	if (one_attack(ch, victim, skill->sn, wield)) {
		if (IS_NPC(victim))
			act("$n knocks $N out cold!", ch, NULL, victim, TO_ROOM);

		stop_fighting(ch, TRUE);
		stop_fighting(victim, TRUE);
		if ((sleep = skill_lookup("sleep")) != NULL) {
			af.where = TO_AFFECTS;
			af.type = sleep->sn;
			af.skill = sleep;
			af.level = ch->level;
			af.duration = (int)(ch->level / 100);
		...
		if (IS_AWAKE(victim)) {
			send_to_char("BLAM....darkness.\n\r", victim);
			act("$N groans, then falls over.", ch, NULL, victim, TO_NOTVICT);
			send_to_char("You knock your victim out cold.\n\r", ch);

			check_improve(ch, skill, TRUE, 10);
...
void do_kick(CHAR_DATA *ch, char *argument)
{
	...
	if ((skill = skill_lookup("kick")) == NULL) {
		send_to_char("Huh?\n\r", ch);
		return;
	}
	...
	if (percent > number_percent()) {
		if (ch->race == (race = race_lookup("centaur"))) {
			kick_chance = number_range(1, 10);
			if (kick_chance == 7) {
