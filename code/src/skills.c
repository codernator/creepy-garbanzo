...
const struct spell_lookup_type spell_lookup_table[] =
{
	...
	{ "create rose",       spell_create_rose       },
	{ "create spring",     spell_create_spring     },
...
*	check_affected
*
*	check to see if a character is affected by a skill by name
***************************************************************************/
...
*	gain_points
*
*	lower the number of creation points - lowers exp
*	per level
***************************************************************************/
static void gain_points(CHAR_DATA *ch, CHAR_DATA *trainer)
{
...
void do_skillset(CHAR_DATA *ch, char *argument)
{
	...
	if (is_help(argument)) {
		send_to_char("syntax:  skillset <skill name>\n\r", ch);
		return;
	}

	if (ch->pcdata->skillset == 0) {
		send_to_char("You do not have any `8skillset`` tokens.\n\r", ch);
		return;
	}

	if (ch->pcdata->train < 2) {
		send_to_char("It costs 2 training sessions to redeem a `8skillset`` token.\n\r", ch);
		return;
	}
	...
	if (learned->percent == 100) {
		send_to_char("That skill is already at `#100`3%``\n\r", ch);
		return;
	}
	...
	sprintf(desc, "%s", trainer->short_descr);
	desc[0] = UPPER(desc[0]);
	sprintf(buf, "%s teaches you advanced %s techniques.\n\r", desc, skill->name);

	send_to_char(buf, ch);

	sprintf(buf, "You are now `1l`!ea`&r`!ne`1d`` at %s.\n\r", skill->name);
	send_to_char(buf, ch);
}
