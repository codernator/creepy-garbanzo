...
void do_ovnum(CHAR_DATA *ch, char *argument)
{
	...
	if (argument != NULL) {
		...
		if (arg[0] == '?' || arg[0] == '\0') {
			...
			(*cmp_fn)(get_obj_index(OBJ_VNUM_MAP), arg, buffer);
			page_to_char(buf_string(buffer), ch);
		} else {
			OBJ_INDEX_DATA *obj;
			char *clr1;
			...
			for (iter = low_vnum; iter <= high_vnum && count < top_obj_index; iter++) {
				if ((obj = get_obj_index(iter)) != NULL) {
					count++;

					if ((*cmp_fn)(obj, arg, NULL)) {
						number++;
